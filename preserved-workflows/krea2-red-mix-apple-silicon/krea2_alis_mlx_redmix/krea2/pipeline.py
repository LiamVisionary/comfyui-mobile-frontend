...
from .transformer import Krea2Config, SingleStreamDiT, fuse_block_projections
...
class Krea2Pipeline:
    ...
    - 'mxfp8'     : MLX MXFP8 transformer weights (28-block attn+mlp @ MXFP8)
    - 'mxfp8-fused': MLX MXFP8 weights with fused QKV/gate and MLP gate/up projections
    ...
    def __init__(self, transformer_path: str | None = None, precision: str = "8bit", base_dir: str | None = None):
        ...
        if precision == "8bit":
            ...
            nn.quantize(m, mode="mxfp8", class_predicate=quantize_bulk)
            m.load_weights(transformer_path, strict=True)
        elif precision == "mxfp8-fused":
            fuse_block_projections(m)
            nn.quantize(m, mode="mxfp8", class_predicate=quantize_bulk)
            m.load_weights(transformer_path, strict=True)
        elif precision == "bf16":
            ...
            raise ValueError(f"precision must be '8bit', 'mixed-4-8', 'mxfp8', 'mxfp8-fused' or 'bf16', got {precision}")
