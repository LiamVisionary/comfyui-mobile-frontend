...
export function sortOutputPickerFiles(
  ...
  return [...files].sort((a, b) => {
    ...
    // The picker is opened from an image/video input, so keep the usable recent
    // media first. Folders remain available underneath instead of burying the
    // newest outputs below directory rows.
